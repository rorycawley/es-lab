...
(defn- source [path] (slurp (io/file "src/lab23" path)))
...
(deftest schemas-live-in-the-shell-test
  (testing "and are used by the driving edge and the store adapter"
    ...
    (is (contains? (requires (source "adapter/intake.clj")) "lab23.schema.command"))
    (is (contains? (requires (source "adapter/postgres.clj")) "lab23.schema.event"))))
...
(deftest the-application-layer-depends-on-ports-not-adapters-test
  (let [app (source "app.clj")]
    (is (contains? (requires app) "lab23.port.driven"))
