...
(def actor {:type "user" :id "USR-83721"})
(def command {:command/id command-1 :command/type :example :command/actor actor :data {}})
(def event {:event/id event-1 :event/type :example-recorded :event/occurred-at t0 :data {:value "kept"}})
...
(deftest event-store-adapters-preserve-facts-and-audit-metadata-test
  (each-adapter (fn [{:keys [store]}]
                  (let [[recorded] (driven/append store stream-1 0 command [event])]
                    ...
                    (is (= actor (json/read-str (json/write-str (get-in recorded [:metadata :actor]))
                                                :key-fn keyword)))
...
(deftest event-store-adapters-enforce-expected-version-test
  (each-adapter (fn [{:keys [store]}]
                  (driven/append store stream-1 0 command [event])
                  (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Concurrent modification"
                                        (driven/append store stream-1 0 command
                                                       [(assoc event :event/id (random-uuid))]))))))
(deftest outbox-adapters-preserve-message-content-test
  (each-adapter (fn [{:keys [outbox]}]
                  (driven/enqueue outbox [message])
                  (let [[recorded] (driven/pending outbox)]
