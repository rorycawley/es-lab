...
(defrecord MemoryStore [state]
  ...
  (stream-version [this stream-id]
    (->> (driven/read-stream this stream-id) (map :stream/version) (apply max 0)))
  ...
  (append [this stream-id expected-version command events]
    (let [actual (driven/stream-version this stream-id)]
            ...
            stamped (vec (map-indexed
                          (fn [i event]
                            (assoc event
                                   ...
                                   ;; Who is answerable for this fact, as an
                                   ;; opaque id and nothing more (lab 1).
                                   ;; Never the token that proved it.
