...
Implementation follows the test layers defined in the Testing Strategy. The
development workflow is:

1. Pull latest trunk before starting any work.
2. Write a failing test that references an acceptance criterion ID.
3. Write the minimal code to pass the test.
4. Commit. Pre-commit hook runs Layer 1 (pure function) tests.
5. Push. Pre-push hook runs Layers 1 + 2.
6. CI runs all layers and blocks merge on failure.

Every commit leaves the system in a releasable state. Work in progress that
is not safe to release is hidden behind a feature flag or kept out of trunk
until it is complete enough to be harmless.
