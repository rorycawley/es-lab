(ns lab27.ordering.api
  ...
  (:require [lab27.ordering.catalog-price-changed :as catalog-price-changed]
            [lab27.ordering.get-order :as get-order]
            [lab27.ordering.place-order :as place-order]
            [lab27.ordering.search-orders :as search-orders]
            [lab27.platform.behaviour :as behaviour]))

(defrecord Ordering [receive place-order get-order search audit-log])
...
(defn new-module
  ...
  (let [audit      (atom [])
    ...
    (->Ordering receive command query search #(deref audit))))
