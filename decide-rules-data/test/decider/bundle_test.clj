(ns decider.bundle-test
  "`resource-paths` is hand-maintained, and everything generic is driven from
   it: `load-all` feeds the generative suite, so a bundle missing from the list
   is a bundle nothing tests. That is a silent failure, and this is what makes
   it a loud one."
  (:require
...
(defn- bundle-files-on-disk
  ...
  []
  (->> (io/file (io/resource bundle/resource-directory))
       .listFiles
       (map #(.getName %))
...
(deftest every-bundle-file-is-listed-and-every-listing-exists
  (let [listed (set (map #(str/replace % (str bundle/resource-directory "/") "")
    ...
    (testing "no bundle file is missing from resource-paths"
      (is (empty? (remove listed on-disk))
          "add it to decider.bundle/resource-paths, or nothing will test it"))
    (testing "no listed path is missing from disk"
      (is (empty? (remove on-disk listed))
          "remove it from decider.bundle/resource-paths"))
    (is (= listed on-disk))))

(deftest resource-paths-has-no-duplicates
  (is (= (count bundle/resource-paths)
         (count (set bundle/resource-paths)))))

(deftest load-all-loads-every-listed-bundle
  (let [loaded (bundle/load-all)]
    (is (= (count bundle/resource-paths) (count loaded)))
...
;; Fixtures live in `test/bundle-fixtures/` and are on the test classpath.
;; Writing them into `resources/semantic-bundles/` at test time would race with
;; the listing test above, which is exactly the kind of tidy idea that makes a
;; suite flaky.
...
(deftest load-prepared-is-load-then-prepare
  ...
  (doseq [path bundle/resource-paths]
    (testing path
