...
(defn config []
  (let [{:keys [container]} @environment
    ...
    {:catalog       {:jdbcUrl url :user "catalog_module" :password "catalog-pass"}
     ...
     :payments      {:jdbcUrl url :user "payments_module" :password "payments-pass"}
     :notifications {:jdbcUrl url :user "notifications_module"
                     :password "notifications-pass"}
     :websub        {:jdbcUrl url :user "websub_module" :password "websub-pass"}}))
  ...
  (let [{:keys [admin]} @environment]
    (jdbc/execute!
     ...
     ["TRUNCATE catalog.product, catalog.outbox, catalog.command_ledger,
                catalog.description_ledger,
                ordering.price_book, ordering.orders, ordering.inbox,
                ordering.outbox,
                payments.inbox, payments.payment, payments.webhook_inbox,
                payments.outbox,
                catalog.delivery, ordering.delivery, payments.delivery,
                catalog.dead_letter, ordering.dead_letter, payments.dead_letter,
                ordering.fulfilment,
                notifications.inbox, notifications.notification,
                websub.public_product, websub.inbox, websub.subscription
        RESTART IDENTITY"])))
