  ...
  (fixture/with-system
    (fn [{:keys [catalog ordering] :as app}]
      ...
      (testing "the outbox crosses the boundary"
        (is (= 1 (count (delivered-to (system/relay-catalog! app) :ordering))))
        (is (= 600
  ...
  (fixture/with-system
    (fn [{:keys [catalog ordering] :as app}]
      ...
      (let [{:keys [message headers]} (first (delivered-to
        ...
        (is (empty? (delivered-to (system/relay-catalog! app) :ordering))
            "Ordering's delivery has already been marked")))))
  ...
  (fixture/with-system
    (fn [{:keys [catalog] :as app}]
      (let [command-id (random-uuid)
        ...
        (let [[{:keys [message]}] (delivered-to (system/relay-catalog! app) :ordering)]
      ...
      (let [request  {:command-id (random-uuid)
        ...
        (let [results (mapv deref attempts)]
          ...
          (is (= 1 (count (set (map :message-id
                                    (:delivered (system/relay-catalog! app))))))))))))
