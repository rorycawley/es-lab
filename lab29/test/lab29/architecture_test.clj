...
(deftest code-is-grouped-by-business-capability-and-use-case-test
  (doseq [path ["src/lab29/catalog/change_price.clj"
                "src/lab29/catalog/get_product.clj"
                "src/lab29/ordering/place_order.clj"
                "src/lab29/ordering/get_order.clj"
                "src/lab29/ordering/catalog_price_changed.clj"]]
...
(deftest modules-communicate-only-through-public-contracts-test
  (testing "Catalog knows nothing about Ordering"
    (doseq [file (clj-files "src/lab29/catalog")
            required (requires (slurp file))]
      (is (not (str/starts-with? required "lab29.ordering"))
          (str (.getName file) " requires " required))))
  (testing "Ordering may know Catalog's contract and nothing behind it"
    (doseq [file (clj-files "src/lab29/ordering")
            required (filter #(str/starts-with? % "lab29.catalog")
                             (requires (slurp file)))]
      (is (= "lab29.catalog.contract" required)
...
(deftest cross-cutting-concerns-wrap-slices-at-the-api-test
  (doseq [api ["src/lab29/catalog/api.clj" "src/lab29/ordering/api.clj"]
...
(deftest the-two-that-do-take-one-half-each-test
  (testing "producing telemetry"
    (let [source (slurp (io/file "src/lab29/platform/telemetry.clj"))]
  ...
  (testing "configuring where it goes"
    (let [source (slurp (io/file "src/lab29/system.clj"))]
...
(deftest the-index-lives-with-the-data-it-indexes-test
  (doseq [path ["src/lab29/catalog/search_products.clj"
                "src/lab29/ordering/search_orders.clj"]]
...
(deftest nothing-can-rank-one-owner-against-the-other-test
  ...
  (doseq [file (clj-files "src/lab29")
    ...
    (is (not (and (contains? required "lab29.catalog.api")
                  (contains? required "lab29.ordering.api")))
...
(deftest each-adapter-names-only-its-own-vendor-test
  (is (nil? (re-find #"(?i)sendgrid"
                     (code-only (slurp (io/file "src/lab29/payments/adapter/stripe.clj"))))))
  (is (nil? (re-find #"(?i)stripe"
                     (code-only (slurp (io/file "src/lab29/notifications/adapter/sendgrid.clj")))))))
...
(deftest the-modules-depend-on-ports-not-adapters-test
  ...
  (doseq [[slice required] [["src/lab29/payments/charge_order.clj" "lab29.payments.port"]
                            ["src/lab29/notifications/send_receipt.clj"
                             "lab29.notifications.port"]]]
...
(deftest every-port-has-more-than-one-implementation-test
  ...
  (doseq [[port implementations]
          [["payments" ["src/lab29/payments/adapter/stripe.clj"
                        "src/lab29/payments/adapter/memory.clj"]]
           ["notifications" ["src/lab29/notifications/adapter/sendgrid.clj"
                             "src/lab29/notifications/adapter/memory.clj"]]]]
...
(deftest each-adapter-declares-what-it-may-retry-test
  ...
  (doseq [path ["src/lab29/payments/adapter/stripe.clj"
                "src/lab29/notifications/adapter/sendgrid.clj"]]
...
(deftest the-relay-policy-is-shared-and-the-sql-is-not-test
  ...
  (let [relay (slurp (io/file "src/lab29/platform/relay.clj"))]
  ...
  (let [schema (slurp (io/file "resources/schema.sql"))]
    (doseq [module ["catalog" "ordering" "payments"]]
      (let [outbox (slurp (io/file (str "src/lab29/" module "/outbox.clj")))]
        (is (str/includes? outbox (str "(def schema \"" module "\")"))
            (str module " does not identify its own schema"))
        (is (str/includes? schema (str "CREATE TABLE " module ".dead_letter"))
...
(deftest the-fake-providers-are-not-an-application-dependency-test
  (doseq [file (clj-files "src/lab29")]
    (is (not (re-find #"fake-stripe|fake-sendgrid|lab29\.chaos" (slurp file)))
...
(deftest webhooks-are-verified-before-they-are-believed-test
  ...
  (let [source (slurp (io/file "src/lab29/http.clj"))
...
(deftest the-pure-rule-stays-pure-test
  ...
  (let [source (defn-body "src/lab29/ordering/place_order.clj" "price-order")]
    (is (some? source))
