...
(defn- stocked-order!
  ...
  (catalog/change-price! catalog {:command-id (random-uuid)
                                  ...
                                  :price-cents 300})
  (doseq [delivery (delivered-to (system/relay-catalog! app) :ordering)]
  ...
  (let [[delivery] (delivered-to (system/relay-ordering! app) :payments)]
    (select-keys delivery [:headers :message])))

(defn- receipt-delivery!
  "Advance a captured payment fact through Ordering's process manager."
  [{:keys [ordering] :as app}]
  (let [[settled] (delivered-to (system/relay-payments! app) :ordering)]
    (ordering/receive! ordering (select-keys settled [:headers :message]))
    (first (delivered-to (system/relay-ordering! app) :notifications))))
  ...
  (let [provider (fake-sendgrid/start!)
    ...
    (try
      (let [order-id (random-uuid)
            delivery (stocked-order! app order-id)]
        (payments/charge! (:payments app) delivery)
        (let [settled (receipt-delivery! app)
              fact-id (get-in settled [:message :data :receipt-id])
...
(deftest a-crash-after-the-email-sent-sends-it-again-test
  ...
  (let [provider (fake-sendgrid/start!)]
    (try
      (let [healthy (sendgrid/emailer {:base-url (:base-url provider) :api-key "SG.lab29"})
            app     (system/start (assoc (postgres/config)
                                         :gateway {:provider :memory}
    ...
    (fn [{:keys [app stripe]}]
      (let [order-id (random-uuid)
        ...
        (let [results (mapv deref attempts)]
          ...
          (is (= "authorized" (:status (:found (payments/get-payment
                                                (:payments app) {:order-id order-id})))))
          (is (= 1 (count (set (map :message-id
                                    (:delivered (system/relay-payments! app))))))
