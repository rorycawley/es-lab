...
| Database migrations | Flyway (as Docker Compose / Kubernetes Job) | Same | Same |
...
| 03 | in progress | `task-persistence` | A user submits a service request through the frontend. The backend validates it, persists it to Postgres via a Flyway migration, and reads it back. Proves durable state and round-trip data flow. Introduces intentful business API endpoints (POST commands and queries), the ports-and-adapters boundary at persistence, and a simple audit log alongside the domain table. A fixed demo-user header (`X-User-Id`) stands in for real auth. |
| 04 | in progress | `legal-facts` | The register IS the legal reality. A company record entered in the register is a legal fact: immutable, traceable to law, and distinct from the application process that created it. Proves legal fact as a first-class design concept using incorporation as the domain. Introduces the full architecture documentation chain — law (Companies Registration Act) → vision and business drivers → domain discovery → 26 ADRs → user stories with acceptance criteria → named tests → CI — and proves that delivery is demonstrably complete against the law. Demonstrates event sourcing, CQRS, FSM-governed aggregates, ports and adapters, TDD with five test layers (pure function / adapter / behavioural / fitness function / contract), trunk-based development, and a bilaterally agreed Definition of Done and acceptance protocol. Architecture documentation lives in `04a-legal-facts/docs/`; the reference implementation lives in `04-legal-facts/`. |
| 05 | planned | `dual-portal-basics` | Two SPAs (customer and backoffice) share one backend; distinct command and query endpoints prove segregated API surfaces without a gateway. |
| 06 | planned | `ports-and-adapters` | The persistence adapter introduced in 03 is extracted behind a Clojure protocol and swapped for a second implementation, proving the domain is fully isolated from infrastructure. |
| 07 | planned | `modular-monolith` | Multiple bounded contexts live in one deployable while remaining strictly isolated from each other. |
