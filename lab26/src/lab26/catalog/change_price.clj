(ns lab26.catalog.change-price
  ...
  (:require [lab26.catalog.contract :as contract]
  ...
  (jdbc/execute-one!
   ...
   ["INSERT INTO catalog.outbox
       (message_id, message_type, fact_id, causation_id, correlation_id,
        traceparent, product_id, product_name, price_cents, published)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, FALSE)"
    message-id
    (str (namespace contract/price-changed-type)
         "/" (name contract/price-changed-type))
    fact-id command-id correlation-id
    ;; The trace this price change happened in, captured here rather than at
    ;; relay time — lab 12's argument for freezing the message with the write,
    ;; applied to the context that explains it.
