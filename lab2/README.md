...
When a policy has to remember where it has got to, it needs state, and *Enterprise Integration Patterns* calls that a **process manager**: a central unit that maintains state and determines the next step from intermediate results. Microsoft's CQRS Journey describes one as routing messages and sometimes translating between message types. The useful boundary is more precise than "no business rules": a coordinator owns the rule that determines **what request follows this fact**, but must not copy the target aggregate's state-dependent acceptance rules. The target's `decide` function remains authoritative for those invariants. [Lab10](../lab10#a-policy-owns-the-reaction-decide-protects-the-target) makes the distinction executable.
