...
A user submits a service request through an Angular 21 SPA. The Clojure backend validates it, persists it to Postgres via Flyway migrations, writes an audit record, serves it back via read queries, and supports Postgres full-text search across titles and descriptions. The frontend reloads the list after each submission and provides active search over submitted requests.

Introduces: durable Postgres persistence, Flyway migrations, generated `tsvector` search columns with a GIN index, intentful business API endpoints (POST commands and queries), ports-and-adapters at the persistence boundary, and a simple audit log.
...
| Frontend server | nginx | Serves static files; proxies `/api/` to the backend without rewriting the path |
| API design | POST JSON for business commands and queries | Intentful — every call names its operation; uniform auth and audit surface |
...
Business API endpoints use `POST` with a JSON body. No business endpoint uses `GET`, `PUT`, `PATCH`, or `DELETE`. Operational and documentation endpoints remain conventional `GET` endpoints (`/health`, `/openapi.json`, `/swagger-ui`).
...
| POST | `/api/queries/search-service-requests` | Return service requests matching a search query |
...
| `http://localhost:4200/api/queries/search-service-requests` | Search query via nginx proxy |
...
| Searchability | Service requests are indexed with Postgres full-text search; title matches are weighted above description matches |

## Fitness criteria

- `docker compose up` completes without error on a machine with only Docker installed
- `POST /api/commands/submit-service-request` with `{"title":"T","description":"D"}` returns `201 Created` with a `request_id`
- `POST /api/queries/list-service-requests` with `{}` returns `200 OK` with a `requests` array
- `POST /api/queries/search-service-requests` with `{"query":"printer"}` returns `200 OK` with a `requests` array
- Search rejects a missing or blank `query` with `422`
- Title matches rank ahead of description matches for the same search term
- Submitting a request then listing returns the submitted request in the list
- Submitting a request then searching returns the submitted request when the query matches the title or description
- A submitted request is still returned after restarting Postgres and the backend
- The Angular app shows a submitted request title in the list after form submission
- `docker compose down` exits cleanly with no project containers remaining
- `bb down` and Docker-backed test runs remove the project Postgres volume, so local data does not leak across runs
...
The command handler validates that title and description are non-blank strings before touching the database. The ports-and-adapters boundary means query construction is confined to one adapter file. `save!` and `audit/record!` execute inside a single JDBC transaction, so if the audit write fails the service request is rolled back.
...
- Search pagination and highlighting
...
- Flyway migration filenames use the `V1__description.sql` convention and are recorded in the `schema_version` metadata table
...
- `audit/record!` is called inside the same transaction as `save!` — audit is mandatory, not optional, and a failed audit write rolls back the service request
