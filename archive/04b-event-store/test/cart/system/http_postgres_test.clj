...
(defn- with-started-system
  "Runs f against a started system and an HttpClient scoped to that instance.

   The client must not outlive the server. A client reused across a restart
   still holds a pooled keep-alive connection to the previous, now-closed
   server, and because the OS is free to hand the next server the same
   ephemeral port, that dead socket looks reusable. The request is then written
   to a closed connection and surfaces as
   'HTTP/1.1 header parser received no bytes' — a flake that depends on which
   port the OS picks, so it reproduces perhaps one run in five."
  [f]
...
(deftest http-service-persists-through-postgres
  (let [cart-id   (str "pg-http-English-购物车-عربة-" (random-uuid))
