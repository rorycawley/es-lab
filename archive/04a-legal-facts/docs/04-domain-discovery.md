...
| **Audit Log** | A separate, human-readable, append-only store of significant Registry decisions. Written atomically with domain events via the in-process event bus (same transaction); if the audit write fails, the whole transaction rolls back. Serves external auditors. Contains full-fidelity data with no masking; access is extremely restricted |
...
| **Command Dispatcher** | Routes an incoming command to exactly one handler (the Decider for the target aggregate). One sender, one handler, one result. This is the Mediator pattern. It is entirely separate from the event bus |
| **In-process Event Bus** | A synchronous publish/subscribe mechanism within the process. One publisher (the Decider), zero or more subscribers (handlers). Publishers do not know who is subscribed. This is pub/sub, not the Mediator pattern. In this system, only the audit writer subscribes to the in-process bus (ADR-0015 Path 1). Projectors and process managers use event store polling instead |
