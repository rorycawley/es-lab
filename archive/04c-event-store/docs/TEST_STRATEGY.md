...
Iteration 1 delivered 14 acceptance cases: all of `UC-01/S01`, all of
...
## Iteration 2 Evidence Plan

Iteration 2 delivers 20 acceptance cases: all of `UC-01/S03`,
`UC-01/S04/TC02` through `TC13`, all of `UC-02/S02`, and all of `UC-02/S05`.
The three closed-cart cases formerly assigned here move to Iteration 4. Test
fixtures may establish state only through mounted public commands; direct event,
projection or row insertion is not valid scenario setup.

Retain the Iteration 1 suites unchanged as regression evidence. Add a shared
`test/cart/acceptance/iteration_two.clj` public-boundary suite that runs against
a newly migrated SQLite database per case and the suite-scoped Testcontainers
PostgreSQL database reset between cases. Memory runs the same command/query
behavior through inbound ports for fast feedback, but SQLite and PostgreSQL
remain the evidence required before a traceability row becomes Verified.

Do not create a PostgreSQL container topology per iteration. One PostgreSQL
acceptance namespace and one suite-scoped PostgreSQL/Flyway fixture run the
shared adapter contract plus every delivered iteration suite, resetting the
five application tables between cases. The dedicated migration suite retains
its separate topology because migrate-twice and schema ownership are different
claims.

The Iteration 2 suites must prove:

- pure removal decisions, fold behavior, zero-line removal, checked limits,
  corrupt-removal failure, cart-view projection and removal-history delta data
- exact sequential replay after a later accepted change and concurrent equal
  removal delivery committing one event and one command result on every adapter
- all invalid quantity variants: zero, negative, above 1000 and non-integer;
  all product-ID variants; first and existing add shapes where the case applies
- all observation-integrity variants: altered, fabricated and different-cart,
  each returning `400 invalid` and never `409 conflict`
- request-ID misuse across command type, cart and business input, plus reuse
  after invalid and business-rejected attempts
- `400` for declared-input failures, `422` for insufficient removal or an add
  that would exceed the held limit, and no request-ID consumption for either
- projection-only views after add and removal, stable represented revision on
  repeated views, UUID-sorted items, and no query-created history
- OpenAPI-conforming `200`, `400`, `409`, `422` and `500` remove responses and
  `200`, `400` and `500` view responses through inbound-port stubs

Every failed command captures durable state before and after the public call.
The comparison includes stream revision and event IDs, cart-view projection,
ordered history projection, accepted-command count and unrelated cart state.
HTTP status alone is not evidence of the no-partial-change requirement.

The existing version 1 migration is sufficient for Iteration 2. Migration tests
continue to require exactly V1 and the same five application tables; adding the
removal event must not introduce an unnecessary V2 migration.
...
| Add/remove/view application behavior | `bb test:slices` |
...
`bb ci` runs static checks, every no-Docker suite, PostgreSQL adapter and
acceptance evidence, and the separate Testcontainers PostgreSQL migration suite.
It must start from disposable infrastructure rather than relying on a
developer's Compose database.
...
Iteration 1 delivered pure domain and observation-codec tests, inbound add/view
port tests, shared persistence port contracts and the first 14 acceptance tests.
Iteration 2 adds removal, strict content validation and complete open-cart view
evidence without weakening those suites. Each later slice adds its acceptance
IDs to executable test names while retaining focused lower-level evidence. A row
in requirements traceability moves from Planned to Verified only when its
public-boundary acceptance test passes against the required persistent adapters.
