...
(deftest the-inbox-makes-contract-republication-idempotent-test
  (fixture/with-system
    (fn [{:keys [catalog ordering] :as app}]
      (change-vanilla! catalog 300)
      (let [{:keys [message headers]} (first (:published (system/relay-catalog! app)))]
        ...
        (is (empty? (:published (system/relay-catalog! app)))
  ...
  (fixture/with-system
    (fn [{:keys [catalog] :as app}]
      (let [command-id (random-uuid)
        ...
        (let [[{:keys [message]}] (:published (system/relay-catalog! app))]
