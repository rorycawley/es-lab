(ns lab28.gateway-contract-test
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [lab28.notifications.port :as email-port]
...
(deftest the-stripe-adapter-actually-sends-the-header-test
  ...
  (let [provider (fake-stripe/start!)]
    (try
      (let [gateway    (stripe/gateway {:base-url (:base-url provider) :api-key "sk_test_lab28"})
...
(deftest an-unreachable-gateway-throws-rather-than-declining-test
  ...
  (let [gateway (stripe/gateway {:base-url "http://localhost:1" :api-key "sk_test_lab28"})]
...
(deftest the-sendgrid-adapter-really-sends-twice-test
  (let [provider (fake-sendgrid/start!)]
    (try
      (let [emailer (sendgrid/emailer {:base-url (:base-url provider) :api-key "SG.lab28"})
