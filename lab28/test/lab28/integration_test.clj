...
(defn- fact-of [app]
  (get-in (first (:published (system/relay-payments! app))) [:message :payload :fact-id]))
...
(deftest modules-know-only-the-contracts-test
  (fixture/with-system
    (fn [{:keys [catalog ordering] :as app}]
      ...
      (let [[{:keys [message]}] (:published (system/relay-ordering! app))]
        (is (= :ordering/order-placed (:message/type message)))
        (is (= #{:fact-id :order-id :product-name :quantity :total-cents
                 :customer-email :payment-method}
               (set (keys (:payload message)))))
        (is (= #{:causation-id :correlation-id} (set (keys (:metadata message))))))
      (let [[{:keys [message]}] (:published (system/relay-payments! app))]
        (is (= :payments/payment-succeeded (:message/type message)))
