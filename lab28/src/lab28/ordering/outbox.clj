(ns lab28.ordering.outbox
  "Ordering's module-owned outbox, new in lab 28."
...
(defn pending
  ...
  (mapv (fn [{:keys [message-id fact-id causation-id correlation-id traceparent
                     order-id product-name quantity total-cents customer-email
                     payment-method attempts]}]
          {:attempts attempts
           :headers (if traceparent {"traceparent" traceparent} {})
           :message (contract/order-placed message-id fact-id causation-id
                                           correlation-id order-id product-name
                                           quantity total-cents customer-email
                                           payment-method)})
        (jdbc/execute!
         datasource
         ["SELECT message_id, fact_id, causation_id, correlation_id, traceparent, attempts,
                  order_id, product_name, quantity, total_cents, customer_email,
                  payment_method
...
(defn mark-published!
  ...
  (jdbc/execute-one!
   datasource
   ["UPDATE ordering.outbox SET published = TRUE WHERE message_id = ?" message-id]))
...
(defn dead-letter!
  ...
  (jdbc/with-transaction [tx datasource]
    (jdbc/execute-one!
     tx
     ["INSERT INTO ordering.dead_letter
    ...
    (jdbc/execute-one!
     tx
     ["UPDATE ordering.outbox SET dead = TRUE, attempts = ?, last_error = ?
        WHERE message_id = ?"
...
(defn dead-letters
  ...
  (jdbc/execute!
   ...
   ["SELECT message_id, message_type, fact_id, attempts, last_error, died_at
       FROM ordering.dead_letter ORDER BY died_at"]
   {:builder-fn rs/as-unqualified-kebab-maps}))
...
(defn revive!
  ...
  (jdbc/with-transaction [tx datasource]
    (when-let [row (jdbc/execute-one!
                    tx
                    ["SELECT message_body FROM ordering.dead_letter
      ...
      (jdbc/execute-one!
       tx
       ["DELETE FROM ordering.dead_letter WHERE message_id = ?" message-id])
