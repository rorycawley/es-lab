...
;; and from any library's. A name with a digit in it would be mangled — `lab28`
...
(def ^:private logger (LoggerFactory/getLogger "lab28"))
...
(defonce ^:private requests
  (delay (instrument/instrument {:name            "lab28.slice.requests"
...
(defn- outcome-of
  ...
  words existed. Lab 28 added `:pending`, which is the one an operator most
  wants to alert on -- work handed to somebody else that never came back."
  [response]
  (name (cond
          ...
          ;; Callback outcomes. A webhook endpoint that reports only "200"
          ;; tells an operator nothing, and these four are exactly the
          ;; distinctions worth alerting on separately.
          (:already-applied response)  :already-applied
