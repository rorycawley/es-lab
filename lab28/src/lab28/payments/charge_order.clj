(ns lab28.payments.charge-order
  "The complete `Charge order` slice: consume an integration event, take money.
  ...
  (:require [lab28.ordering.contract :as ordering-contract]
            [lab28.payments.announce :as announce]
...
(def Request
  [:map {:closed true}
   ...
   [:message ordering-contract/OrderPlaced]])
...
(defn handle!
  ...
  (let [{:keys [correlation-id]} (:metadata message)
        {:keys [fact-id order-id total-cents customer-email payment-method]}
        (:payload message)
