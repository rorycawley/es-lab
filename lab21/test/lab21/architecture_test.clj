(ns lab21.architecture-test
  ...
  These checks are intentionally coupled to an architectural policy. They are
  not the business regression suite: `app-test` protects use-case behaviour
  while internals change. If the policy itself changes, these checks should be
  ...
  (testing "search the source for a concrete adapter and find one file"
    (let [files (->> (file-seq (io/file "src/lab21"))
          ...
          namers (reduce (fn [found file]
                           (cond-> found
                             (str/includes? (slurp file) "lab21.adapter.postgres")
                             (conj file)))
                         []
                         files)]
