(ns lab21.adapter-test
  "The driven-adapter contract, separate from business behaviour.

  This suite asks whether an EventStore and Outbox preserve their port
  semantics. It runs against memory and real Postgres, but it contains no
  truck rule: infrastructure mapping failures should not masquerade as a
  business regression."
  ...
  (each-adapter
   (fn [{:keys [store]}]
     (let [[recorded] (port/append store stream-1 0 command [event])]
       ...
       (is (= (str command-1)
              (str (get-in recorded [:metadata :causation-id]))))
