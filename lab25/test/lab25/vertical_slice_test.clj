    ...
    (fn [{:keys [catalog ordering] :as app}]
      ...
      (let [message (get-in (first (system/relay-catalog! app)) [:message])]
        (is (= (get-in message [:payload :fact-id])
               (:duplicate (ordering/receive! ordering
  ...
  (fixture/with-system
    (fn [{:keys [catalog] :as app}]
      (let [command-id (random-uuid)
        ...
        (let [[delivery] (system/relay-catalog! app)
...
(deftest closed-integration-contracts-reject-unknown-fields-test
  (fixture/with-system
    (fn [{:keys [ordering]}]
      (let [message (assoc (catalog-contract/price-changed
                            (random-uuid) (random-uuid)
                            (random-uuid) (random-uuid)
                            vanilla "vanilla" 300)
                           :internal/debug true)]
        (is (= :malformed (:rejected (ordering/receive! ordering message))))
        (is (= :price-unavailable
               (:rejected
